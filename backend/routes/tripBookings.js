...
router.post('/', auth, async (req, res) => {
  try {
    ...
    await booking.save();
    await booking.populate(['tripDetails.destinationId', 'hotelBookings.hotel']);
...
router.get('/:bookingId', auth, async (req, res) => {
  try {
    const booking = await TripBooking.findOne({
      ...
      { path: 'tripDetails.destinationId', select: 'name images location attractions activities' },
...
async function generateItinerary(destination, duration, tripDetails) {
  ...
  const hotels = await Hotel.find({
    $or: [
      { location: { $regex: destination?.location || tripDetails.destination, $options: 'i' } },
      { 'address.city': { $regex: destination?.district || 'Andhra Pradesh', $options: 'i' } }
    ]
  }).limit(3);
  ...
  for (let day = 1; day <= duration; day++) {
    ...
    const dayItinerary = {
      day,
      date: currentDate,
      activities: generateDayActivities(destination, day, tripDetails.interests),
      accommodation: day < duration ? {
        hotel: hotels[0]?._id,
        checkIn: currentDate,
...
function generateDayActivities(destination, day, interests) {
  ...
  return baseActivities.map((attraction, index) => ({
    ...
    description: attraction.description || `Enjoy ${attraction.name || attraction}`,
