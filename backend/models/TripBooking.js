...
const tripBookingSchema = new mongoose.Schema({
  bookingId: {
    ...
    required: true,
    unique: true,
    default: () => 'TRB' + Date.now() + Math.random().toString(36).substr(2, 5).toUpperCase()
  },
  user: {
...
tripBookingSchema.methods.generateConfirmation = function() {
  return {
    ...
    tripSummary: {
      ...
      dates: `${this.tripDetails.startDate.toDateString()} to ${this.tripDetails.endDate.toDateString()}`,
